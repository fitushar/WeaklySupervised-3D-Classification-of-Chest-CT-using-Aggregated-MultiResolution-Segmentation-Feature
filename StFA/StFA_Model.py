...
def StFAModel_withDenseVnet(DenseVnet3D_Model_Path,Input_shape,num_classes_clf,num_classes_for_seg):
    ...
    #-| Concatenate the FAs
    #FA_concatination=concatenate([upsampled_F,last_predict],axis=-1) #not using last layeroutput
    FA_concatination=tf.math.reduce_mean(upsampled_F,axis=-1)
    FA_concatination=(FA_concatination-tf.math.reduce_min(FA_concatination))/(tf.math.reduce_max(FA_concatination)-tf.math.reduce_min(FA_concatination))
    FA_concatination=tf.expand_dims(FA_concatination, axis=-1)
    #-|| DyFA- Pass the Concatinated Feature to 1x1x1 convolution to get a 1 channel Volume.
    #DyFA=tf.keras.layers.Conv3D(1, 1, name='DyFA')(FA_concatination)

    #-|| Making a HxWxDx2 channel Input data for the DyFA Classification Model
    DyFA_INPUT=concatenate([FA_concatination,inputs],axis=-1)
